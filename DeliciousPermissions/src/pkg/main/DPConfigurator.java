...
public class DPConfigurator {
	...
	public DPConfigurator(DeliciousPermissions plugin) {
	...
	public void load() {
		...
		if (!configFile.exists()) {
			try {
				...
				DeliciousPermissions.logger.log(Level.SEVERE, "Error creating a new config.yml", ex);
			}
		}
		...
		try {
			...
			try {
				...
				DeliciousPermissions.logger.log(Level.SEVERE, "Missing or corrupt 'allow_commandblocks' node. Using default settings", ex);
			}
			
			try {
				opOverride = (Boolean) config.get("opOverrides");
			} catch (Exception ex) {
				DeliciousPermissions.logger.log(Level.SEVERE, "Missing or corrupt 'opOverrides' node. Using default settings", ex);
			}
			
			try {
				toggleValidate = (Boolean) config.get("validate_toggle");
			} catch (Exception ex) {
				DeliciousPermissions.logger.log(Level.SEVERE, "Missing or corrupt 'validate_toggle' node. Using default settings", ex);
			}
			...
			try {
				...
				try {
					...
					DeliciousPermissions.logger.log(Level.SEVERE, "Missing or corrupt 'minutes' node. Using default setting", ex);
				}
				
				try {
					backupDuration = (Integer) save.get("hours");
				} catch (Exception ex) {
					DeliciousPermissions.logger.log(Level.SEVERE, "Missing or corrupt 'hours' node. Using default setting", ex);
				}
				
			} catch (Exception ex) {
				DeliciousPermissions.logger.log(Level.SEVERE, "Missing or corrupt 'data' node. Using default settings", ex);
			}
			 ...
			 */
			DeliciousPermissions.logger.log(Level.SEVERE, "There are errors in your config.yml. Using default settings", ex);
	...
	public void adjustLoggerLevel() {
		...
		try {
			DeliciousPermissions.logger.setLevel(Level.parse(loggerLevel));
			return;
		} catch (Exception e) {
		}

		DeliciousPermissions.logger.setLevel(Level.INFO);
	}
