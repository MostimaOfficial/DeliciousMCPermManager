 ...
 */
public class PermissionHandler extends PermissionsReaderInterface {
	 ...
	 * @param holder
	 */
	public PermissionHandler(WorldDataHolder holder) {
	...
	public Set<String> getAllPlayersPermissions(String userName, Boolean includeChildren) {
		...
		for (String group : getGroups(userName)) {
			...
			if (!alreadyProcessed.contains(group)) {
				...
				if (group.startsWith("g:") && DeliciousPermissions.getGlobalGroups().hasGroup(group)) {
					// GlobalGroups
					groupPermArray = populatePerms(DeliciousPermissions.getGlobalGroups().getGroupsPermissions(group), includeChildren);
	...
	private boolean wildcardNegation(Set<String> playerPermArray, String node) {
		...
		for (String part : parts) {
			...
			if (playerPermArray.contains("-" + builder.toString())) {
				DeliciousPermissions.logger.fine("Wildcard Negation found for " + node);
				return true;
			}
		 ...
		 */
		DeliciousPermissions.logger.fine("No Negation found for " + node);
		return false;
	...
	private Set<String> populatePerms(List<String> permsList, boolean includeChildren) {
		...
		if (perms.contains("*")) {
			permArray.addAll(DeliciousPermissions.BukkitPermissions.getAllRegisteredPermissions(includeChildren));
			allPerms = true;
			perms.remove("*");
		...
		for (String perm : perms) {
			...
			if (!permArray.contains(perm)) {
				...
				if ((includeChildren) || (negated && allPerms)) {
					...
					Map<String, Boolean> children = DeliciousPermissions.BukkitPermissions.getAllChildren((negated ? perm.substring(1) : perm), new LinkedHashSet<String>());
