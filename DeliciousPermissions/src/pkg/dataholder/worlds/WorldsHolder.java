...
public class WorldsHolder {
	...
	private DeliciousPermissions plugin;
	 ...
	 * @param plugin
	 */
	public WorldsHolder(DeliciousPermissions plugin) {
	...
	private void loadAllSearchedWorlds() {
		...
		for (World world : plugin.getServer().getWorlds()) {
			DeliciousPermissions.logger.log(Level.FINE, "Checking data for " + world.getName() + ".");
			if ((!worldsData.containsKey(world.getName().toLowerCase())) && ((!mirrorsGroup.containsKey(world.getName().toLowerCase())) || (!mirrorsUser.containsKey(world.getName().toLowerCase())))) {
				...
				DeliciousPermissions.logger.log(Level.FINE, "Creating folders for " + world.getName() + ".");
		...
		for (File folder : worldsFolder.listFiles()) {
			if (folder.isDirectory() && !folder.getName().startsWith(".")) {
				DeliciousPermissions.logger.info("World Found: " + folder.getName());
		...
		if (mirrorsMap != null) {
				...
				if (mirrorsMap.get(source) instanceof ArrayList) {
					...
					for (Object o : mirrorList) {
						...
						if (!world.equalsIgnoreCase(serverDefaultWorldName)) {
							...
							DeliciousPermissions.logger.log(Level.WARNING, "Mirroring error with " + o.toString() + ". Recursive loop detected!");
					}
					...
					for (Object key : subSection.keySet()) {
						...
						if (!((String)key).equalsIgnoreCase(serverDefaultWorldName)) {
							...
							if (subSection.get(key) instanceof ArrayList) {
								...
								for (Object o : mirrorList) {
									...
									if (type.equals("groups")) {
										...
										DeliciousPermissions.logger.log(Level.FINE, "Adding groups mirror for " + key + ".");
									}

									if (type.equals("users")) {
										mirrorsUser.put(((String)key).toLowerCase(), getWorldData(source).getName());
										DeliciousPermissions.logger.log(Level.FINE, "Adding users mirror for " + key + ".");
									}
								}
							...
							DeliciousPermissions.logger.log(Level.WARNING, "Mirroring error with " + (String)key + ". Recursive loop detected!");
						}
			...
			for (String world : mirroredWorlds) {
				if (!worldsData.containsKey(world.toLowerCase())) {
					DeliciousPermissions.logger.log(Level.FINE, "No data for " + world + ".");
	...
	public void reloadAll() {
		...
		// Load global groups
		DeliciousPermissions.getGlobalGroups().load();
	...
	public boolean saveChanges(boolean overwrite) {
		...
		// Write Global Groups
		if (DeliciousPermissions.getGlobalGroups().haveGroupsChanged()) {
			DeliciousPermissions.getGlobalGroups().writeGroups(overwrite);
		} else {
			if (DeliciousPermissions.getGlobalGroups().getTimeStampGroups() < DeliciousPermissions.getGlobalGroups().getGlobalGroupsFile().lastModified()) {
		...
		for (OverloadedWorldHolder w : worldsData.values()) {
			...
			if (w == null) {
				DeliciousPermissions.logger.severe("WHAT HAPPENED?");
				continue;
			}
			if (!mirrorsGroup.containsKey(w.getName().toLowerCase()))
				if (w.haveGroupsChanged()) {
					if (overwrite || (!overwrite && (w.getTimeStampGroups() >= w.getGroupsFile().lastModified()))) {
						...
						// Newer file found.
						DeliciousPermissions.logger.log(Level.WARNING, "Newer Groups file found for " + w.getName() + ", but we have local changes!");
			...
			if (!mirrorsUser.containsKey(w.getName().toLowerCase()))
				if (w.haveUsersChanged()) {
					if (overwrite || (!overwrite && (w.getTimeStampUsers() >= w.getUsersFile().lastModified()))) {
						...
						// Newer file found.
						DeliciousPermissions.logger.log(Level.WARNING, "Newer Users file found for " + w.getName() + ", but we have local changes!");
						throw new IllegalStateException("Unable to save unless you issue a '/mansave force'");
					}
				} else {
					//Check for newer file as no local changes.
	...
	private void backupFile(OverloadedWorldHolder w, Boolean groups) {
		...
		try {
			...
			DeliciousPermissions.logger.log(Level.SEVERE, null, ex);
		}
	}
	...
	public OverloadedWorldHolder getWorldData(String worldName) {
		...
		if (worldsData.containsKey("all_unnamed_worlds")) {
			DeliciousPermissions.logger.finest("Requested world " + worldName + " not found or badly mirrored. Returning all_unnamed_worlds world...");
		...
		DeliciousPermissions.logger.finest("Requested world " + worldName + " not found or badly mirrored. Returning default world...");
	 ...
	 */
	public PermissionHandler getWorldPermissions(String worldName) {
	 ...
	 * @return AnjoPermissionsHandler
	 */
	public PermissionHandler getWorldPermissions(Player player) {
	 ...
	 */
	public PermissionHandler getWorldPermissionsByPlayerName(String playerName) {
	...
	public void loadWorld(String worldName, Boolean isMirror) {
		...
		DeliciousPermissions.logger.finest("Trying to load world " + worldName + "...");
		...
		if ((isMirror) || (thisWorldFolder.exists() && thisWorldFolder.isDirectory())) {
			...
			if (thisWorldData != null) {
				DeliciousPermissions.logger.finest("Successful load of world " + worldName + "...");
